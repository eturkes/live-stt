...
## Status
- Phase: **T9 agent-runnable evaluation + long-form robustness** (planned 2026-07-02, user directive after the first live smoke). T8 closed at T8.6 without a phase-review — the field smoke pivoted priorities.
- Trigger: first live-mic smoke (user, 2026-07-02) — long continuous speech loses content (truncated fragments, e.g. a 2-char segment) + `drop=35`; user cannot judge JA accuracy → the accuracy loop moves agent-side. Both defects reproduced deterministically in the planning session (§ Open evidence block).
...
Planning evidence (2026-07-02 session: scratchpad experiments on the real corpus + source-verified research; T9.1's builder re-derives the audio):
...
- Offline decode deletes content wholesale on long segments: k2v2 on a 36.5 s continuous real-speech concat → deletions 54/226 ref chars (24 %; whole utterances vanish, incl. the first 3.4 s of a 20 s segment). Sweeping max_speech 5–12 s barely helps on dip-less audio (segments stay 9–16 s; D≈48). parakeet: D=24 flat to 20 s (loses only quiet-short utterances + the cut-straddled one) — engine-dependent robustness. Lit corroboration: offline attention WER 3.9→12.6→31 % at 20/30/40 s. D-010's "synthetic TTS collapses decode" was this, misattributed (re-attribute in T9.7).
...
- **T9.1 — Continuous-speech stressor corpus + fires-validation.** OPEN. Builder script in `tests/` (deterministic from the cached CV clips, no network): silero-trim each clip to its speech extent, crossfade-join (~10 ms), cycle to ≥35 s plus a ~20 s medium variant; reference = plain concatenation of `ja_ref`s; WAVs → the gitignored cache, manifest (refs + recipe) beside `real_clips.json`. Accept: dry-run shows segment count < clip count (raised-threshold cut fired), ≥1 segment >15 s, k2v2 deletion-rate >10 % (planning repro: 24 %).
- **T9.2 — CER harness + committed baseline.** OPEN. Pure fns: normalize = NFKC → lower → drop Unicode M/S/P categories → strip all whitespace (the kotoba-whisper/BasicTextNormalizer leaderboard recipe) + Levenshtein S/D/I via backtrace; CER=(S+D+I)/N, guard N=0; long-form scoring = concat-all-hyp vs concat-all-ref, one global alignment (segmentation-independent). Numeral folding stays OUT of the primary metric (七 vs 7 counts as S; a second lenient figure is allowed only if reported alongside, never silently). Unit-test with fixed vectors — zero new deps. Score (engine × clip) over the short corpus + T9.1 stressor; include RTF-vs-length (5/10/20/40 s, both engines) + decode-viability at 40 s. Commit the baseline table (deletion-rate = headline for missed content; insertion-rate catches duplication). Accept: both-engine baselines recorded; k2v2 stressor deletion ≥10 % confirmed; short-clip CER consistent with existing goldens.
- **T9.3 — Paced-replay backpressure harness.** OPEN. Drive the REAL `worker` + a real bounded `audio_q` on a virtual clock: chunk-duration knob (~10–100 ms) sets the arrival timeline; decode cost injected by patching the executor/`_decode` to advance the virtual clock — deterministic queue-depth-over-time + drop count, no wall-clock, no reimplemented drop policy (D-014). Accept: a documented worst-case (long segment × small chunks × slow-decode knob) yields drops>0 at `AUDIO_QUEUE_MAX=100` (defect-B repro); the short corpus shows zero drops.
- **T9.4 — Accuracy fix, CER-gated (needs T9.1+T9.2).** OPEN. Cheapest sanctioned lever first: expose `max_speech_duration` (+ raised-mode interplay) as top-of-file constants and sweep ~5–12 s (sherpa's own `generate-subtitles.py` precedent) — planning data predicts partial help on dip-less audio → escalate to decode-side chunking: split popped segments >N s at the lowest-RMS window (ring re-slice, small overlap, text merge). Per-engine numbers; revisit the default engine (parakeet materially more long-form robust) → amend D-010 if flipped. Accept: stressor deletion-rate ≤2× the per-utterance baseline under the shipped default config; short-clip goldens unchanged or improved; live path bit-identical for segments <N.
- **T9.5 — Backpressure fix (needs T9.3).** OPEN. Real cause = single-consumer serialization (`worker` stops draining while awaiting a decode — the decode already runs off-loop). Fix: two coroutines — drain+VAD feeder → bounded segment queue → sequential decode consumer; denominate audio headroom in seconds, not chunks. Preserve: `on_segment` contract (replay output byte-identical on the short corpus pre/post), shutdown handshake (evict-then-put sentinel, Ctrl+C flush order, translator-drains-last). Accept: T9.3 worst-case → zero drops + bounded depth; full gate green; adds a user-smoke item (Ctrl+C mid-decode) to § Smoke.
- **T9.6 — Genuine long-form corpus (best-effort, off the critical path; anytime after T9.2).** OPEN. LibriVox/Kokoro-Speech JA chapter (archive.org direct MP3, public domain) + aligned Aozora text, or a better found source; score with the T9.2 harness (real-speech dips → validates T9.4's levers beyond the pathological crossfade stressor). No /rows path exists for long-form (reazonspeech gated=auto; YODAS2 needs load_dataset+trust_remote_code; FLEURS /rows broken). Failure is an acceptable outcome — record it. Cheap bonus if useful: ungated short sets `japanese-asr/ja_asr.jsut_basic5000`, `ja_asr.reazonspeech_test` via the proven /rows pattern.
- **T9.7 — Closeout (last).** OPEN. Regenerate both-engine goldens + diff-review (separate intended segmentation shifts from regressions); README: long-form behavior + the EN leg needs `codex` on the PATH of the machine running live-stt; memory: re-attribute D-010's synthetic-TTS-collapse claim to long-segment offline-decode degradation, fold T9 lessons; roadmap ledger + § Smoke refresh (agent-covered vs user-only).
...
- Model/architecture upgrades, benchmark-gated on the T9.2 harness (evaluate only if T9.4's levers miss target, or as a future quality phase): streaming zipformer `ar_en_id_ja_ru_th_vi_zh-2025-02-10` (online transducer, ~259 MB, runs on sherpa-onnx 1.13.2 — removes VAD segmentation from the accuracy path entirely; JA quality unverified, 8-language dilution risk); Nemotron-3.5 streaming JA (needs sherpa ≥1.13.3); newer offline JA: `zipformer-ja-en-2025-01-17`, Moonshine-JA, SenseVoice zh-en-ja-ko-yue.
