...
P-022 was CLOSED as a REFUSAL, re-derived over 1409 live captions across 7 sessions. Its own premise
does not survive that re-derivation: of the four captions with 21 ≤ span < 40, `つもい`×10 (session 6
n=103, span 30) is a 30-character loop inside **301 characters of genuine speech**, and the screen
drops WHOLE — this entry called all three decode loops, which is true of the repetitions and false of
the captions, so "loop or speech" was the wrong question to adjudicate. Sparing n=103 and still
catching n=213 gives an admissible band of 31..36, and the tripling invariant (`unit*3 < bound`, which
keeps a 13-character phrase said three times out of the screen) floors the bound at 40 and excludes
that band whole. Forgone by refusing: exactly one caption, session 1 n=213. Evidence and the ruling →
`asr-pipeline.md`; re-open only by redesigning the screen to count repeats per unit size rather than
characters.
