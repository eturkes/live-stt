...
- T8.2 ✓ Live-mic smoke + soak checklist (`.claude/rules/live-smoke.md`) — 7-item live pass + soak watching only in-code observables (meter `q=`/`drop=`, `TRANSLATE_ROTATE_TURNS`, `account/rateLimits/read` quota) → L-020. Runnable, not yet user-passed.
