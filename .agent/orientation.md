...
| `.serena/` | Headroom/Serena LSP state (Headroom compresses everything the agent reads — CLAUDE.md). `project.yml` = tracked LSP config; the nested `.serena/.gitignore` (committed as is) ignores `cache/` + `project.local.yml` (`memories/` is empty + un-ignored, Serena memory disabled globally, but stays Read-denied), and `project.yml`'s `ignored_paths` mirrors the non-gitignored deny-list (`uv.lock`, `LICENSE`) — all also deny-listed → the agent ignores them (D-013). The project memory system is `.agent/`, **not** `.serena/memories/`. |
